...
{
    class Fun4simple : IFun
    {
        PointD[] A;
        PointD[] B;

        private double[] w = new double[] { 1, 1, 0.01, 10000, 10000, 1, 1, 0.01 };
        internal Fun4simple(PointD[] A, PointD[] B)
        {
            this.A = A;
            this.B = B;
        }
        ...
        public double F(int index, Vector x)
        {
            ...
            if (index % 2 == 0)
                ...
                return (x[5] * A[i].x + x[6] * A[i].y + x[7]) / (1 + x[3] * A[i].x + x[4] * A[i].y) - B[i].y;
        }
